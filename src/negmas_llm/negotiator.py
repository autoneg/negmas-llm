...
class LLMNegotiator(SAOCallNegotiator, ABC):
    ...
    Args:
        ...
        raise_on_parsing_error: If True, raise a ValueError when the LLM returns
            an invalid outcome (e.g., None values or values not in outcome space).
            If False (default), log a warning and set the outcome to None
            (making it an information-only message).
    ...
    def _parse_llm_response(
        ...
        # Validate the outcome before returning
    ...
    def _validate_outcome(self, outcome: Outcome) -> Outcome | None:
        """Validate and fix an outcome to ensure it's compatible with the outcome space.

        Args:
            outcome: The outcome to validate.

        Returns:
            A valid outcome tuple, or None if the outcome cannot be fixed.
        ...
        # Check for None values in the outcome
        ...
        # If we have outcome space info, validate against it
        if self.nmi is not None and self.nmi.outcome_space is not None:
            try:
                # Check if outcome is valid in the outcome space
